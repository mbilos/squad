...
class BiDAF_ConvInput:
    ...
    def input_encoder(self):
        ...
        with tf.variable_scope('conv-1') as scope:
            c_conv_1 = tf.layers.conv1d(c_h2, self.config.embed_size, kernel_size=5, padding='SAME', activation=tf.nn.relu)
            q_conv_1 = tf.layers.conv1d(q_h2, self.config.embed_size, kernel_size=5, padding='SAME', activation=tf.nn.relu, reuse=True)

            c_conv_1 = tf.layers.dropout(c_conv_1, rate=self.config.dropout, training=self.config.training)
            q_conv_1 = tf.layers.dropout(q_conv_1, rate=self.config.dropout, training=self.config.training)

            c_conv_1 = util.gated_connection(c_h2, c_conv_1)
            q_conv_1 = util.gated_connection(q_h2, q_conv_1, reuse=True)

        with tf.variable_scope('conv-2') as scope:
            c_conv_2 = tf.layers.conv1d(c_conv_1, self.config.embed_size, kernel_size=5, padding='SAME', activation=tf.nn.relu)
            q_conv_2 = tf.layers.conv1d(q_conv_1, self.config.embed_size, kernel_size=5, padding='SAME', activation=tf.nn.relu, reuse=True)

            c_conv_2 = tf.layers.dropout(c_conv_2, rate=self.config.dropout, training=self.config.training)
            q_conv_2 = tf.layers.dropout(q_conv_2, rate=self.config.dropout, training=self.config.training)

            c_conv_2 = util.gated_connection(c_conv_1, c_conv_1)
            q_conv_2 = util.gated_connection(q_conv_1, q_conv_1, reuse=True)

        return c_conv_2, q_conv_2
