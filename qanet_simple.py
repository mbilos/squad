    ...
    def encoder(self, inputs, num_blocks, num_convolutions, kernel, length=None, scope='encoder', reuse=None):
        ...
        def self_att(x, i, scope='self-attention', reuse=None):
            with tf.variable_scope(scope, reuse=reuse) as scope:
                ...
                att = tf.matmul(ln, ln, transpose_b=True) - 1e30 * tf.eye(length)
                att = tf.nn.softmax(att, -1)
                res = tf.matmul(att, ln)
        ...
        def residual_block(x, j):
            with tf.variable_scope('residual-block-%d' %j):
                ...
                conv = tf.layers.conv1d(ln, self.config.filters, kernel, padding='same', activation=tf.nn.relu, reuse=reuse)
                return conv
    ...
    def input_embedding(self):
        with tf.variable_scope('input-embedding'):
            ...
            with tf.variable_scope('highway'):
                c_h1 = tf.layers.conv1d(c, self.config.embed_size, 1, padding='same')
                c_h1 = util.gated_connection(c, c_h1)

                q_h1 = tf.layers.conv1d(q, self.config.embed_size, 1, padding='same', reuse=True)
                q_h1 = util.gated_connection(q, q_h1, reuse=True)

            c_h2 = tf.layers.conv1d(c_h1, self.config.filters, 1, padding='same')
            q_h2 = tf.layers.conv1d(q_h1, self.config.filters, 1, padding='same', reuse=True)
